...
export async function POST(
  ...
  if (!file && !(isResubmit && keepExisting)) {
    return NextResponse.json({ error: 'Screenshot is required' }, { status: 400 })
  }

  // Build screenshot data (new upload or keep existing)
  ...
  if (file) {
    ...
    screenshotData = `data:${file.type};base64,${base64}`
  }
  ...
  if (!screenshotData) return NextResponse.json({ error: 'Screenshot is required' }, { status: 400 })
