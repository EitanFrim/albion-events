    ...
    const material = new THREE.ShaderMaterial({
      ...
      vertexShader: `
        ...
        void main() {
          vNormal = normal;
          vPosition = position;
          float displacement = snoise(position * 1.5 + time * 0.3) * 0.15;
      ...
      fragmentShader: `
        ...
        void main() {
          ...
          // Fresnel for glow edges
          float fresnel = 1.0 - dot(normal, vec3(0.0, 0.0, 1.0));
          fresnel = pow(fresnel, 2.0);

          // Mix colors based on displacement
          float mixFactor = smoothstep(-0.1, 0.1, vDisplacement);
          vec3 color = mix(color1, color2, mixFactor);

          vec3 finalColor = color * (diffuse * 0.6 + 0.4) + color * fresnel * 0.5;
          gl_FragColor = vec4(finalColor, 0.8 * (fresnel * 0.5 + 0.5));
        }
      `,
      wireframe: true,
      transparent: true,
