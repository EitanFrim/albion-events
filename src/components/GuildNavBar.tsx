...
interface Props {
  ...
  membership: { role: string; status: string; balance: number; siphonedEnergy: number }
  totalGuildBalance?: number
  totalGuildEnergy?: number
}
...
export function GuildNavBar({ guild, membership, totalGuildBalance = 0, totalGuildEnergy = 0 }: Props) {
  ...
  const mgmtActive = pathname.startsWith(`${base}/admin/players`) ||
    ...
    pathname.startsWith(`${base}/admin/siphoned-energy`) ||
  ...
  const mgmtLinks = [
    ...
    { href: `${base}/admin/siphoned-energy`, label: 'Siphoned Energy', icon: 'M13 10V3L4 14h7v7l9-11h-7z', show: true },
  ...
  return (
    ...
    >
      <div className="max-w-7xl mx-auto px-4 h-full flex items-center justify-between gap-4">
                      ...
                      <div className="w-64 rounded-xl border border-white/[0.08] shadow-[0_16px_48px_rgba(0,0,0,0.5)] overflow-hidden backdrop-blur-xl" style={{ background: 'rgba(17, 17, 24, 0.85)' }}>
                        ...
                        <div className="px-4 py-3 border-b border-border-subtle bg-gradient-to-r from-teal-500/[0.06] to-transparent">
                          ...
                          <div className="flex items-center gap-2">
                            ...
                            <span className={`text-lg font-display font-700 ${totalGuildEnergy < 0 ? 'text-red-400' : 'text-teal-400'}`}>
                              {totalGuildEnergy.toLocaleString()}
                            </span>
        ...
        {/* Right: how-to + refresh + balance + user menu */}
        <div className="flex items-center gap-2 flex-shrink-0">
          {/* How To — officer+ only */}
          {isOfficerPlus && (
            <Link
              href={`${base}/guide`}
              className={`hidden md:flex items-center gap-1.5 px-2.5 py-1.5 rounded-lg text-sm font-body transition-colors ${
          ...
          {!isPending && (
            <Link href={`${base}/my-siphoned-energy`} className="flex items-center gap-1.5 px-2 py-1 rounded-lg bg-teal-500/5 border border-teal-500/15 hover:bg-teal-500/10 transition-colors">
              ...
              <span className={`text-sm font-mono font-medium ${membership.siphonedEnergy < 0 ? 'text-red-400' : 'text-teal-400'}`}>
                {membership.siphonedEnergy.toLocaleString()}
              </span>
            </Link>
          )}
      ...
      {/* IGN not set banner */}
      {!isPending && session && !session.user.inGameName && (
